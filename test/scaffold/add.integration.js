...
describe('#add', function() {
  ...
  before(function(done) {
    mkdirp(testDir + '/s0/s1', function(err) {
      ...
      fs.writeFile(
        testDir + '/s0/s1/bitcore-node-pivx.json',
  ...
  describe('will modify scaffold files', function() {
    ...
    it('will update bitcore-node-pivx.json services', function(done) {
      var callCount = 0;
      var oldPackage = {
        dependencies: {
          'bitcore-lib-pivx': '^v0.13.17',
          'bitcore-node-pivx': '^v0.2.0'
        }
      };
      var spawn = sinon.stub().returns({
        stdout: {
      ...
      addtest({
        ...
        var configPath = path.resolve(testDir, 's0/s1/bitcore-node-pivx.json');
