...
function checkConfigVersion2(fullConfig) {
  ...
  if (!datadirUndefined || addressDefined || dbDefined) {
    ...
    console.warn('\nConfiguration file is not compatible with this version. \n' +
                 ...
                 'There are changes necessary in both bitcoin.conf and bitcore-node-pivx.json. \n\n' +
                 'To upgrade please see the details below and documentation at: \n' +
                 'https://github.com/bitpay/bitcore-node-pivx/blob/bitcoind/docs/upgrade.md \n');
        ...
        servicesConfig: {
          bitcoind: {
            spawn: {
              ...
              exec: path.resolve(__dirname, '../../bin/pivxd')
            }
          }
        }
      };
...
function start(options) {
  ...
  fullConfig.path = path.resolve(options.path, './bitcore-node-pivx.json');
...
function loadModule(req, service) {
  try {
    // first try in the built-in bitcore-node-pivx services directory
