...
function add(options, done) {
  ...
  var bitcoreConfigPath = path.resolve(configPath, 'bitcore-node-pivx.json');
  var packagePath = path.resolve(configPath, 'package.json');

  if (!fs.existsSync(bitcoreConfigPath) || !fs.existsSync(packagePath)) {
    return done(
      new Error('Directory does not have a bitcore-node-pivx.json and/or package.json file.')
    );
  }
  ...
  async.eachSeries(
    ...
    function(service, next) {
      ...
      addService(configPath, service, function(err) {
        ...
        console.log('new dependencies lenght: '+newDependencies.length);
        $.checkState(newDependencies.length === 1);
        oldPackage = updatedPackage;
        var serviceName = newDependencies[0];

        // add service to bitcore-node-pivx.json
