...
var BitcoinRPC = require('bitcoind-rpc-pivx');
...
Bitcoin.prototype._loadSpawnConfiguration = function(node) {
  ...
  $.checkArgument(this.options.spawn, 'Please specify "spawn" in pivxd config options');
  $.checkArgument(this.options.spawn.datadir, 'Please specify "spawn.datadir" in pivxd config options');
  $.checkArgument(this.options.spawn.exec, 'Please specify "spawn.exec" in pivxd config options');

  this._expandRelativeDatadir();

  var spawnOptions = this.options.spawn;
  var configPath = path.resolve(spawnOptions.datadir, './pivx.conf');
...
Bitcoin.prototype._checkConfigIndexes = function(spawnConfig, node) {
  $.checkState(
    ...
    '"txindex" option is required in order to use transaction query features of bitcore-node-pivx. ' +
  ...
  $.checkState(
    ...
    '"addressindex" option is required in order to use address query features of bitcore-node-pivx. ' +
  ...
  $.checkState(
    ...
    '"spentindex" option is required in order to use spent info query features of bitcore-node-pivx. ' +
  ...
  if (spawnConfig.reindex && spawnConfig.reindex === 1) {
    log.warn('Reindex option is currently enabled. This means that pivxd is undergoing a reindex. ' +
             ...
             'of bitcore-node-pivx services will start.');
    node._reindex = true;
  }
};
...
Bitcoin.prototype._getNetworkConfigPath = function() {
  var networkPath;
  if (this.node.network === bitcore.Networks.testnet) {
    networkPath = 'testnet3/pivx.conf';
    if (this.node.network.regtestEnabled) {
      networkPath = 'regtest/pivx.conf';
    }
  }
  return networkPath;
};
    ...
    if(!self.node.stopping) {
      self.syncPercentage(function(err, percentage) {
        if (err) {
          ...
          log.info('PIVX Height:', self.height, 'Percentage:', percentage.toFixed(2));
        }
      });
    }
  }
  ...
  for (var i = 0; i < addresses.length; i++) {
    ...
    if(this.subscriptions.address[address]) {
      ...
      for(var j = 0; j < emitters.length; j++) {
        emitters[j].emit('pivxd/addresstxid', {
          address: address,
          txid: txid
        });
      }
...
Bitcoin.prototype._zmqTransactionHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactions.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.rawtransaction.length; i++) {
      this.subscriptions.rawtransaction[i].emit('pivxd/rawtransaction', message.toString('hex'));
    }
...
Bitcoin.prototype._zmqTransactionLockHandler = function(node, message) {
  ...
  if (!self.zmqKnownTransactionLocks.get(id)) {
    ...
    for (var i = 0; i < this.subscriptions.transactionlock.length; i++) {
      this.subscriptions.transactionlock[i].emit('pivxd/transactionlock', message.toString('hex'));
    }
  ...
  if (node._reindex) {
    interval = setInterval(function() {
      node.client.getBlockchainInfo(function(err, response) {
        ...
        log.info('PIVX Core Daemon Reindex Percentage: ' + percentSynced.toFixed(2));
...
Bitcoin.prototype._stopSpawnedBitcoin = function(callback) {
  var self = this;
  var spawnOptions = this.options.spawn;
  var pidPath = spawnOptions.datadir + '/pivxd.pid';
  ...
  function stopProcess() {
    fs.readFile(pidPath, 'utf8', function(err, pid) {
      ...
      try {
        log.warn('Stopping existing spawned pivx process with pid: ' + pid);
...
Bitcoin.prototype._spawnChildProcess = function(callback) {
  ...
  self._stopSpawnedBitcoin(function(err) {
    ...
    log.info('Starting pivx process');
    ...
    self.spawn.process.once('exit', function(code) {
      if (!self.node.stopping) {
        log.warn('PIVX process unexpectedly exited with code:', code);
        log.warn('Restarting pivx child process in ' + self.spawnRestartTime + 'ms');
        setTimeout(function() {
          self._spawnChildProcess(function(err) {
            if (err) {
              return self.emit('error', err);
            }
            log.warn('PIVX process restarted');
          });
    ...
    async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
      ...
      if (exitShutdown) {
        return callback(new Error('Stopping while trying to spawn pivxd.'));
      }
...
Bitcoin.prototype._connectProcess = function(config, callback) {
  ...
  async.retry({times: 60, interval: self.startRetryInterval}, function(done) {
    ...
    if (exitShutdown) {
      return callback(new Error('Stopping while trying to connect to pivxd.'));
    }
...
Bitcoin.prototype.start = function(callback) {
  ...
  async.series([
    ...
    if (self.nodes.length === 0) {
      return callback(new Error('PIVX configuration options "spawn" or "connect" are expected'));
    }
...
Bitcoin.prototype._maybeGetBlockHash = function(blockArg, callback) {
  ...
  if (_.isNumber(blockArg) || (blockArg.length < 40 && /^[0-9]+$/.test(blockArg))) {
    self._tryAllClients(function(client, done) {
        console.log("calling client getBlockHash..");
      client.getBlockHash(blockArg, function(err, response) {
        console.log("getBLockHash result: "+ response);
        if (err) {
          return done(self._wrapRPCError(err));
        }
...
Bitcoin.prototype.getBlockHeader = function(blockArg, callback) {
  var self = this;
  console.log('Entering getBlockHeader for hash: '+blockArg);
  function queryHeader(err, blockhash) {
    ...
    self._tryAllClients(function(client, done) {
      client.getBlockHeader(blockhash, function(err, response) {
        ...
        var result = response.result;
        console.log('block header version:'+result.version+", height: "+result.height);
        var header = {
          hash: result.hash,
          version: result.version,
          confirmations: result.confirmations,
  ...
  if (this.spawn && this.spawn.process) {
    ...
    this.spawn.process.once('exit', function(code) {
      if (!exited) {
        exited = true;
        if (code !== 0) {
          var error = new Error('pivxd spawned process exited with status code: ' + code);
          error.code = code;
    ...
    setTimeout(function() {
      if (!exited) {
        exited = true;
        return callback(new Error('pivxd process did not exit'));
      }
